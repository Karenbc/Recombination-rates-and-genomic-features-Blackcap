...
#SBATCH --job-name=hardfilter_subset_indel
...
#SBATCH --error=<path_to_dir>/stdout/hardfilter_subset_indel.%J.err
#SBATCH --output=<path_to_dir>/stdout/hardfilter_subset_indel.%J.out
...
# Step 1:
# reset the filters according to the decisions made through the, indel specific, html report made by 7.hard_filter.Rmd
...
java -Xms42G -Xmx42G -jar ${gatk4} VariantFiltration -R ${ref} -V final_indel.vcf.gz -O hard_filter_indel.vcf.gz --filter-name "Low_QD" --filter-expression "QD < 2.5" --filter-name "high_FS" --filter-expression "FS > 200.0" --filter-name "Low_MQ" --filter-expression "MQ < 40.0" --filter-name "Low_ReadPosRankSum" --filter-expression "ReadPosRankSum < -20.0" 2> ${log}/final_indel_hard_filter.log

# Checkpoint 1
date
echo 'Checkpoint 1'

if grep -i exception ${log}/final_indel_hard_filter.log ; then 
    echo "failed"
    exit 1
else echo "no problem occurred when resetting the filters"
fi

if test -s hard_filter_indel.vcf.gz ; then
	echo "the hard filter indel vcf exists and contains data"
else echo "failed"
	exit 1
fi
...
date 
echo 'Step 2: validating the hard_filter_indel vcf'

java -Xms42G -Xmx42G -jar ${gatk4} ValidateVariants -R ${ref} -V hard_filter_indel.vcf.gz -L ${pipeline}/whole_genome_intervals.list --warn-on-errors 2> ${log}/hard_filter_indel_valivar.log

# Checkpoint: 2
date 
echo 'Checkpoint 2'

if grep -i exception ${log}/hard_filter_indel_valivar.log ; then 
    echo "failed"
    exit 1
...
# Step 3:
# parse the vcf with the filters set into selectvariants, to remove all sites that do not meet the hard filters.
date 
echo 'Step 3: selecting the INDELs that passed the hardfilters'

java -Xms42G -Xmx42G -jar ${gatk4} SelectVariants -R ${ref} -V hard_filter_indel.vcf.gz --exclude-filtered -O indel.vcf.gz 2> ${log}/indel_remove_hard_filter.log

# Checkpoint 3
date 
echo 'Checkpoint 3'

if grep -i exception ${log}/indel_remove_hard_filter.log ; then 
    echo "failed"
    exit 1
else echo "no problem occured when removing variants"
fi

if test -s indel.vcf.gz ; then
    echo "the indel vcf exists and contains data"
else echo "failed"
    exit 1
fi
...
java -Xms42G -Xmx42G -jar ${gatk4} ValidateVariants -R ${ref} -V indel.vcf.gz -L ${pipeline}/whole_genome_intervals.list --warn-on-errors 2> ${log}/indel_valivar.log

# Checkpoint 4
date
echo 'Checkpoint 4'

if grep -i exception ${log}/indel_valivar.log ; then 
    echo "failed"
    exit 1
else echo "the variants are valid"
fi

echo 'the INDEL vcf is hard-filtered and ready for analyses'
