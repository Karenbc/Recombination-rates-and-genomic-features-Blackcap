...
#SBATCH --job-name=hardfilter_subset_all_sites
...
#SBATCH --error=<path_to_dir>/stdout/hardfilter_subset_all_sites.%J.err
#SBATCH --output=<path_to_dir>/stdout/hardfilter_subset_all_sites.%J.out
...
# runtime requirement is 240 hours, this should provide enough time for hardfiltering based on the parameters found in previous step.
# Be aware that the files created are very big (the aim is to have all callable sites in this vcf (except for INDELs)).
...
# Step 1:
# reset the filters according to the decisions made through the, snp specific, html report made by 7.hard_filter.Rmd -> an all-sites report should also be generated to know what the snp filters specifically do. -> following the SNP specific filters is specific to the analysis to be preformed on this data.
...
java -Xms92G -Xmx92G -jar ${gatk4} VariantFiltration -R ${ref} -V final_all_sites.vcf.gz -O hard_filter_all_sites.vcf.gz --filter-name "Low_QD" --filter-expression "QD < 2.5" --filter-name "high_FS" --filter-expression "FS > 45.0" --filter-name "High_SOR" --filter-expression "SOR > 3.0" --filter-name "Low_MQ" --filter-expression "MQ < 40.0" --filter-name "Low_MQRankSum" --filter-expression "MQRankSum < -12.5" --filter-name "Low_ReadPosRankSum" --filter-expression "ReadPosRankSum < -8.0" 2> ${log}/final_all_sites_hard_filter.log

# Checkpoint 1
date
echo 'Checkpoint 1'

if grep -i exception ${log}/final_all_sites_hard_filter.log ; then 
    echo "failed"
    exit 1
else echo "no problem occurred when resetting the filters"
fi

if test -s hard_filter_all_sites.vcf.gz ; then
	echo "the hard filter all_sites vcf exists and contains data"
else echo "failed"
	exit 1
fi
...
date 
echo 'Step 2: validating the hard_filter_all_sites vcf'

java -Xms92G -Xmx92G -jar ${gatk4} ValidateVariants -R ${ref} -V hard_filter_all_sites.vcf.gz -L ${pipeline}/whole_genome_intervals.list --warn-on-errors 2> ${log}/hard_filter_all_sites_valivar.log

# Checkpoint: 2
date 
echo 'Checkpoint 2'

if grep -i exception ${log}/hard_filter_all_sites_valivar.log ; then 
    echo "failed"
    exit 1
...
java -Xms92G -Xmx92G -jar ${gatk4} SelectVariants -R ${ref} -V hard_filter_all_sites.vcf.gz --exclude-filtered -O all_sites.vcf.gz 2> ${log}/all_sites_remove_hard_filter.log

# Checkpoint 3
date 
echo 'Checkpoint 3'

if grep -i exception ${log}/all_sites_remove_hard_filter.log ; then 
    echo "failed"
    exit 1
else echo "no problem occured when removing variants"
fi

if test -s all_sites.vcf.gz ; then
    echo "the all_sites vcf exists and contains data"
else echo "failed"
    exit 1
fi
...
date
echo 'Step 4: validating the all_sites vcf'

java -Xms92G -Xmx92G -jar ${gatk4} ValidateVariants -R ${ref} -V all_sites.vcf.gz -L ${pipeline}/whole_genome_intervals.list --warn-on-errors 2> ${log}/all_sites_valivar.log

# Checkpoint 4
date
echo 'Checkpoint 4'

if grep -i exception ${log}/all_sites_valivar.log ; then 
    echo "failed"
    exit 1
else echo "the variants are valid"
fi

echo 'the all_sites vcf is hard-filtered and ready for analyses'
