...
for my $file ( @files ) {
    ...
    my $sbatch_out = "$file.genotypegvcfs.sh";
    ...
    system "mv *genotypegvcfs.sh <path_to_dir>/scripts";
} 

# Here the actual bash script starts as will be made by the above perl command
# Here the genome is split into 10 chunks (approx. equal), as provided from the previous script: 3.combinegvcfs_script.pl 
...
# this is computationally light, time is more the limiting factor than memory.
...
#SBATCH --job-name=genotype_!file!
...
#SBATCH --error=<path_to_dir>/stdout/genotype_!file!.%J.err
#SBATCH --output=<path_to_dir>/stdout/genotype_!file!.%J.out
...
# Step 1:
# genotypegvcfs is best run from the beegfs, this will speed up the process therefore the files were created in this directory, with the previous script. the output will be directly placed into personal home directory.
date
echo 'Step 1: genotypegvcfs, running in the beegfs, output in personal home directory'

# first go to beegfs directory
cd ${beegfs}


java -Xms40G -Xmx40G -jar ${gatk4} GenotypeGVCFs -R ${ref} -V ${name}.g.vcf.gz -O ${vcf}/${name}.vcf.gz 2> ${log}/${name}_genotype.log
...
if grep -i exception ${log}/${name}_genotype.log ; then 
    echo "failed"
    exit 1
else echo "no problem occurred while running genotypegvcfs"
fi

if test -s ${name}.vcf.gz ; then
	echo "combined vcf exists and contained data"
else echo "failed"
	exit 1
fi
...
date
echo 'Step 2: validating the combined vcf'
...
java -Xms40G -Xmx40G -jar ${gatk4} ValidateVariants -R ${ref} -V ${name}.vcf.gz -L ${pipeline}/${name}_intervals.list --warn-on-errors 2> ${log}/${name}_genotype_valivar.log

# Checkpoint: 2
date
echo 'Checkpoint 2'

if grep -i exception ${log}/${name}_genotype_valivar.log ; then 
    echo "failed"
    exit 1
else echo "the variants are valid"
fi

echo 'vcf has been made and validatevariants has run, check manually the warnings'
