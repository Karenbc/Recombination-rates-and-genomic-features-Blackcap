...
# runtime requirement is 22 hours, this should provide enough time for subsetting into INDELsand making the table
...
date
echo 'Step 1: subsetting final vcf into final INDEL vcf'

# change into the correct directory
cd ${vcf}

java -Xms44G -Xmx44G -jar ${gatk4} SelectVariants -R ${ref} -V final.vcf.gz -select-type INDEL -O final_indel.vcf.gz 2> ${log}/final_vcf_subset_indel.log

# Checkpoint: 1
date
echo 'Checkpoint 1'

if grep -i exception ${log}/final_vcf_subset_indel.log ; then 
    echo "failed"
    exit 1
else echo "no problem occurred when subsetting into INDELs"
fi

if test -s final_indel.vcf.gz ; then
	echo "the final indel vcf exists and contains data"
else echo "failed"
	exit 1
fi
...
date
echo " Step 2: validating the final indel vcf"
...
java -Xms44G -Xmx44G -jar ${gatk4} ValidateVariants -R ${ref} -V final_indel.vcf.gz -L ${pipeline}/whole_genome_intervals.list --warn-on-errors 2> ${log}/final_indel_valivar.log

# Checkpoint: 2
date
echo 'Checkpoint 2'

if grep -i exception ${log}/final_indel_valivar.log ; then 
    echo "failed"
    exit 1
...
java -Xms44G -Xmx44G -jar ${gatk4} VariantsToTable -R ${ref} -V final_indel.vcf.gz -O indel.table -F ID -F CHROM -F POS -F FILTER -F QD -F QUAL -F DP -F AD -F FS -F MQ -F MQRankSum -F ReadPosRankSum -F SOR -F NO-CALL -F TRANSITION -F HET -F NCALLED -F MULTI-ALLELIC -raw

# Checkpoint: 3
date
echo 'Checkpoint 3'

if test -s indel.table ; then
	echo "the indel table exists and contains data"
else echo "failed"
	exit 1
fi


echo "subsetted the vcf in INDELs and created a table for the next step of checking hardfilters in R"
